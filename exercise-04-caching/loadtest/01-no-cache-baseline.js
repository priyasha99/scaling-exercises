/**
 * Test 1: Baseline WITHOUT Redis caching
 * ============================================
 * Run this BEFORE enabling caching to establish a baseline.
 * Same load profile as Exercise 03 Test 2 (500 VUs).
 *
 * This test uses the replicas compose file but with caching
 * disabled (you can disable by stopping Redis or by running
 * without @EnableCaching). Or simply use Exercise 03's results
 * as the baseline.
 *
 * For a fair comparison, run against the replicas setup:
 *   docker compose -f docker-compose.replicas.yml up --build
 *   K6_WEB_DASHBOARD=true k6 run 01-no-cache-baseline.js
 *
 * NOTE: To test without cache, you can flush Redis between runs:
 *   docker compose -f docker-compose.replicas.yml exec redis redis-cli FLUSHALL
 *
 * This test intentionally uses random parameters to minimize
 * cache hits, simulating a cold-cache / no-cache scenario.
 */
...
export default function () {
    ...
    if (roll < 0.10) {
        ...
        // Use random suffix to bust cache
