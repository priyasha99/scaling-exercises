...
# Make 5 requests — watch X-Server-Id change, X-Auth-User stays "admin"
for i in {1..5}; do
  curl -si http://localhost/api/auth/verify \
    -H "Authorization: Bearer $TOKEN" 2>/dev/null | grep -E "X-Server-Id|X-Auth-User"
  echo "---"
done
```
...
_Run the tests and fill in your actual numbers._
...
Expected: JWT adds negligible overhead. The signing/verification is sub-millisecond, invisible compared to database queries and cache operations.
