/**
 * Exercise 06 - Test 1: Sync Baseline
 * ====================================
 * Same as Exercise 05's authenticated load test, but specifically
 * for comparing against async mode. Run this with ASYNC_ENABLED=false.
 *
 * This creates products synchronously — the API waits for the DB
 * insert and cache eviction before returning 201.
 *
 * KEY METRICS TO WATCH:
 *   - http_req_duration for POST requests (write latency)
 *   - error_rate (are writes timing out under load?)
 *   - req/s throughput
 *
 * Run:
 *   # First, start with sync mode:
 *   ASYNC_ENABLED=false docker compose up --build
 *   # Then run:
 *   K6_WEB_DASHBOARD=true k6 run 01-sync-baseline.js
 */
...
const writesCompleted = new Counter('writes_completed');
...
export default function (data) {
    ...
    if (rand < 0.30) {
        // 30% writes — higher write ratio to stress-test sync path
        const cat = CATEGORIES[Math.floor(Math.random() * CATEGORIES.length)];
        const product = {
            name: `Sync Product ${Date.now()}-${Math.floor(Math.random() * 10000)}`,
            description: 'Created during sync baseline test',
        ...
        // 25% category stats (expensive computation)
        const cat = CATEGORIES[Math.floor(Math.random() * CATEGORIES.length)];
