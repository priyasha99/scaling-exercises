...
public class ProductConsumer {
    ...
    public void handleProductCreation(ProductMessage message) {
        ...
        try {
            ...
            // Route to correct shard based on category
            if (shardingService.isEnabled()) {
                ShardContext.setCurrentShard(
                        shardingService.getShardIdForCategory(message.getCategory()));
            }
            ...
            // Clear shard context after DB write — cache eviction uses Redis, not the shard
            ShardContext.clear();
